...
from shapely.geometry import Point # Required for creating points
...
TARGET_CRS = "EPSG:3006" # Define target CRS for distance calculation (SWEREF99 TM)

# --- Helper Functions ---
# (Include all helpers: safe_float_to_int, check_hisco_range, is_relevant_person, 
#  extract_birth_decade, classify_education, get_location_details, 
#  get_overseas_experience, parse_json_string, get_hisco_major_group_label,
#  standardize_education_data)
    ...
    for filename in all_files:
        ...
        try:
            ...
            person_record: Dict[str, Any] = {
                ...
                '_education_raw': education_entries, '_career_raw': career_entries, # Keep as lists
             }
...
def calculate_education_networks(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not all(col in df.columns for col in required_cols):
         ...
         df['edu_network_size'] = 0; df['edu_network_wl_birth_prop'] = np.nan; return df
    for idx, row in df.iterrows():
        person_id = row['person_id']; born_wl = row['birth_parish_is_western_line'] # True/False/None
        ...
        for edu_entry in education_list:
             inst_std = edu_entry.get("institution_standardized"); year_str = edu_entry.get("year"); year = pd.to_numeric(year_str, errors='coerce')
             if pd.notna(inst_std) and pd.notna(year) and isinstance(inst_std, str): institution_lookup[inst_std].append((person_id, int(year), born_wl))
    ...
    for idx, row in df.iterrows():
        ...
        for edu_entry in education_list: 
            inst_std = edu_entry.get("institution_standardized"); year_str = edu_entry.get("year"); year = pd.to_numeric(year_str, errors='coerce')
            if pd.notna(inst_std) and pd.notna(year) and isinstance(inst_std, str): year = int(year); temp_primary_edu.append({'inst': inst_std, 'year': year}); latest_grad_year = max(latest_grad_year, year)
        ...
        for primary_edu in primary_edu_entries: 
             ...
             current_network_size = len(cohort); person_cohort_sizes.append(current_network_size)
             if current_network_size > 0:
                 known_status_peers = [p for p in cohort if p['born_wl'] is not None]; wl_peers = sum(1 for peer in known_status_peers if peer['born_wl'] is True)
                 wl_proportion = wl_peers / len(known_status_peers) if known_status_peers else 0; person_cohort_wl_props.append(wl_proportion)
    ...
    df['edu_network_size'] = network_sizes; df['edu_network_wl_birth_prop'] = network_wl_props
    logging.info("Finished calculating network variables.")
    return df

# --- **** NEW: Function to Calculate Distances **** ---
def calculate_distances(df: pd.DataFrame, target_crs: str = TARGET_CRS) -> pd.DataFrame:
    """Calculates distances between birth, current, and first job locations."""
    logging.info("Calculating distance variables...")
    
    # --- Find First Job Location ---
    first_job_locs = []
    for idx, row in df.iterrows():
        first_job_loc = {"lat": None, "lon": None}
        try:
            # Use the raw career list stored in the dict before DF creation
            career_list = row['_career_raw'] 
            if isinstance(career_list, list):
                # Sort by start year, handling missing start years
                valid_jobs = sorted(
                    [j for j in career_list if pd.notna(pd.to_numeric(j.get('start_year'), errors='coerce'))],
                    key=lambda x: pd.to_numeric(x['start_year'])
                )
                if valid_jobs:
                    # Get location of the earliest job
                    loc_details = get_location_details(valid_jobs[0].get('location'))
                    first_job_loc = {"lat": loc_details['lat'], "lon": loc_details['lon']}
        except Exception as e:
            logging.warning(f"Could not process career data for {row.get('person_id')}: {e}")
            
        first_job_locs.append(first_job_loc)
        
    # Add first job coords to DataFrame temporarily
    df['_first_job_lat'] = [loc['lat'] for loc in first_job_locs]
    df['_first_job_lon'] = [loc['lon'] for loc in first_job_locs]

    # --- Prepare Points for Distance Calculation ---
    # Create Point geometries only for rows with valid coordinates
    def create_point(lon, lat):
        if pd.notna(lon) and pd.notna(lat):
            try: return Point(lon, lat)
            except Exception: return None
        return None

    df['birth_geom'] = df.apply(lambda row: create_point(row['birth_location_lon'], row['birth_location_lat']), axis=1)
    df['current_geom'] = df.apply(lambda row: create_point(row['current_location_lon'], row['current_location_lat']), axis=1)
    df['first_job_geom'] = df.apply(lambda row: create_point(row['_first_job_lon'], row['_first_job_lat']), axis=1)

    # Create GeoDataFrames (only non-null geometries)
    valid_birth = df[df['birth_geom'].notna()].copy()
    valid_current = df[df['current_geom'].notna()].copy()
    valid_first_job = df[df['first_job_geom'].notna()].copy()

    dist_birth_current = pd.Series(index=df.index, dtype=float)
    dist_birth_first_job = pd.Series(index=df.index, dtype=float)

    if not valid_birth.empty:
        try:
            gdf_birth = gpd.GeoDataFrame(valid_birth, geometry='birth_geom', crs="EPSG:4326").to_crs(target_crs)
            
            if not valid_current.empty:
                gdf_current = gpd.GeoDataFrame(valid_current, geometry='current_geom', crs="EPSG:4326").to_crs(target_crs)
                # Align indices for direct calculation
                gdf_current_aligned = gdf_current.reindex(gdf_birth.index)
                # Calculate distance (meters), result aligns with gdf_birth index
                distances = gdf_birth.distance(gdf_current_aligned, align=False) 
                dist_birth_current.update(distances / 1000) # Convert to km
                logging.info(f"Calculated birth-to-current distance for {distances.notna().sum()} people.")

            if not valid_first_job.empty:
                gdf_first_job = gpd.GeoDataFrame(valid_first_job, geometry='first_job_geom', crs="EPSG:4326").to_crs(target_crs)
                 # Align indices for direct calculation
                gdf_first_job_aligned = gdf_first_job.reindex(gdf_birth.index)
                 # Calculate distance (meters)
                distances = gdf_birth.distance(gdf_first_job_aligned, align=False)
                dist_birth_first_job.update(distances / 1000) # Convert to km
                logging.info(f"Calculated birth-to-first-job distance for {distances.notna().sum()} people.")

        except Exception as e:
            logging.error(f"Error during distance calculation or CRS projection: {e}")

    # Assign calculated distances back to the main DataFrame
    df['dist_birth_to_current_km'] = dist_birth_current
    df['dist_birth_to_first_job_km'] = dist_birth_first_job
    
    # Drop temporary geometry columns
    df = df.drop(columns=['birth_geom', 'current_geom', 'first_job_geom', '_first_job_lat', '_first_job_lon'], errors='ignore')
    
    logging.info("Finished distance calculation.")
    return df
...
def main() -> int:
    ...
    if not os.path.exists(INSTITUTION_MAPPING_PATH): logging.warning(f"Institution mapping file not found: {INSTITUTION_MAPPING_PATH}.")
        ...
        try:
            with open(INSTITUTION_MAPPING_PATH, 'r', encoding='utf-8') as f: inst_mapping = json.load(f); logging.info(f"Loaded institution mapping.")
    ...
    try:
        ...
        required_shp_cols = [SHP_PARISH_CODE_COL, SHP_PARISH_NAME_COL, SHP_WESTERN_LINE_COL, SHP_GEOMETRY_COL]
    ...
    for person in persons_data_list: 
        ...
        person['birth_parish_code'] = birth_parish_info.get('parish_code'); person['birth_parish_name'] = birth_parish_info.get('parish_name'); person['birth_parish_is_western_line'] = birth_parish_info.get('is_western_line') 
        ...
        if isinstance(raw_edu_list, list):
            standardized_edu_list = standardize_education_data(raw_edu_list, inst_mapping) if inst_mapping else raw_edu_list; person['education_standardized'] = json.dumps(standardized_edu_list, ensure_ascii=False); person['_education_raw'] = json.dumps(raw_edu_list, ensure_ascii=False)
        else: person['education_standardized'] = '[]'; person['_education_raw'] = '[]'
    ...
    if final_df.empty: logging.error("No data processed into DataFrame."); return 1

    # --- Calculate Derived Variables (incl. NEW Distances) ---
    logging.info("Creating derived variables (father class, studied at, cohort, distances, networks)...")
    ...
    bins = [0, 1880, 1900, 1920, np.inf]; labels = ['<1880', '1880-1899', '1900-1919', '1920+']; final_df['birth_cohort'] = pd.cut(final_df['birth_decade'], bins=bins, labels=labels, right=False).astype('category')
    
    # Calculate Distances BEFORE calculating networks (as network calc needs the DF)
    final_df = calculate_distances(final_df) 
    ...
    # Define independent variables (Including NEW distance variables)
    independent_vars = [
        "C(father_hisco_major_group_label, Treatment(reference='Unknown'))", 'C(birth_cohort)', 
        'edu_technical', 'edu_business', 'edu_other_higher', 'career_has_overseas', 
        'career_has_us', 'board_membership_count', 'studied_kth', 'studied_chalmers', 
        'studied_hhs', 'studied_foreign','edu_network_size', 
        'dist_birth_to_current_km', 'dist_birth_to_first_job_km' # Added distances
    ]
    
    # Handle remaining NaNs (Impute distances with median? Or drop?)
    # Example: Impute distances
    for col in ['dist_birth_to_current_km', 'dist_birth_to_first_job_km']:
        median_val = df_model1[col].median()
        df_model1[col].fillna(median_val, inplace=True)
        median_val_m2 = df_model2[col].median() # Recalculate for potentially different dataset
        df_model2[col].fillna(median_val_m2, inplace=True)
        logging.info(f"Imputed NaN in '{col}' with median: {median_val:.1f} (M1), {median_val_m2:.1f} (M2)")
        
    # Drop rows if essential categoricals are NA (less likely now)
    essential_predictors = ['father_hisco_major_group_label', 'birth_cohort'] 
    ...
    # --- Define and Run Probit Models ---
    formula_rhs = " + ".join(independent_vars)
    
    # Model 1
    if not df_model1.empty:
        logging.info("\n--- Running Probit Model 1: Predicting Birth in WL Parish ---")
        formula1 = f"dep_var_birth_wl ~ {formula_rhs}"; logging.info(f"Formula: {formula1}")
        if df_model1['dep_var_birth_wl'].nunique() < 2: logging.error("M1 DV has only one outcome.")
        else:
            try:
                ...
                if np.isnan(results1.llf): logging.warning("M1 NaN log-likelihood.")
                if not results1.mle_retvals['converged']: logging.warning("M1 failed to converge.")
            except Exception as e: logging.error(f"Error fitting Model 1: {e}", exc_info=True)
    else: logging.warning("Skipping Model 1: No data.")

    # Model 2
    if not df_model2.empty:
        logging.info("\n--- Running Probit Model 2: Predicting Work in WL before 1930 ---")
        formula2 = f"dep_var_work_wl ~ {formula_rhs}"; logging.info(f"Formula: {formula2}")
        if df_model2['dep_var_work_wl'].nunique() < 2: logging.error("M2 DV has only one outcome.")
        else:
            try:
                probit_model2 = smf.probit(formula=formula2, data=df_model2)
                results2 = probit_model2.fit(maxiter=200) 
                print("\n--- Probit Model 2 Summary ---"); print(results2.summary())
                if not results2.mle_retvals['converged']: logging.warning("M2 failed to converge.")
            except Exception as e: logging.error(f"Error fitting Model 2: {e}", exc_info=True)
    else: logging.warning("Skipping Model 2: No data.")
...
if __name__ == "__main__":
    ...
    from shapely.geometry import Point # Ensure Point is imported
    main()