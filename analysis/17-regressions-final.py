...
# --- Helper Functions (Assume defined correctly as in previous versions) ---
...
def extract_persons_data() -> List[Dict[str, Any]]:
    ...
    # (Same as previous version - reads JSONs, extracts fields including _education_raw, _career_raw as lists)
...
def map_unique_locations(locations_df: pd.DataFrame, parishes_gdf: gpd.GeoDataFrame) -> Dict[Tuple[float, float], Dict[str, Any]]:
    ...
    # (Same as previous version - uses correct SHP cols, returns lookup with True/False/None)
...
def calculate_education_networks(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # (Same as previous version - calculates size and prop, uses COHORT_YEAR_WINDOW)
    ...
    for idx, row in df.iterrows():
        ...
        for primary_edu in primary_edu_entries: # Calculate for latest year entries
             ...
             if current_network_size > 0:
                 ...
                 wl_proportion = wl_peers / len(known_status_peers) if known_status_peers else 0 
    ...
    df['edu_network_wl_birth_prop'] = network_wl_props # Contains NaN where imputation needed
...
def main() -> int:
    ...
    for person in persons_data_list: # Use initial list of dicts
    ...
    for person in persons_data_list: # Iterate through original list of dicts
        # Assign spatial info using lookup
        ...
        # Calculate career flags using lookup
        ...
        # Standardize Education using raw list & keep both raw and standardized as JSON strings
        raw_edu_list = person.get('_education_raw', []) # Use raw list from dict
        if isinstance(raw_edu_list, list):
            ...
            person['_education_raw'] = json.dumps(raw_edu_list, ensure_ascii=False)
        else: person['education_standardized'] = '[]'; person['_education_raw'] = '[]'

        # Store raw career as JSON string
        ...
        processed_persons_list.append(person) # Append the *processed dictionary*
        
    # --- 6. Create DataFrame and Add Final Calculated Columns ---
    final_df = pd.DataFrame(processed_persons_list) # Create DF from processed list
    if final_df.empty:
        logging.error("No data processed. Exiting.")
        return 1

    logging.info("Creating derived variables (father class, studied at, cohort, networks)...")
    ...
    # **** REMOVED 'edu_network_wl_birth_prop' ****
    independent_vars = [
        # **** MODIFIED: Set baseline for father's group ****
        "C(father_hisco_major_group_label)", 
        'C(birth_cohort)', 
        'edu_technical', 'edu_business', 'edu_other_higher', 'career_has_overseas', 
        'career_has_us', 'board_membership_count', 'studied_kth', 
        'studied_chalmers', 'studied_hhs', 'studied_foreign',
        'edu_network_size' # Keep network size
        # 'edu_network_wl_birth_prop' # REMOVED as requested
    ]
    
    # Handle NaNs in remaining predictors
    # Impute network size with median (if needed, though less likely to be NaN)
    median_net_size1 = df_model1['edu_network_size'].median()
    median_net_size2 = df_model2['edu_network_size'].median()
    logging.info(f"Imputing NaN in 'edu_network_size' with median: Model 1={median_net_size1}, Model 2={median_net_size2}")
    df_model1['edu_network_size'].fillna(median_net_size1, inplace=True)
    df_model2['edu_network_size'].fillna(median_net_size2, inplace=True)

    # Define essential predictors where rows should be dropped if NA (if any besides DV)
    essential_predictors = ['father_hisco_major_group_label', 'birth_cohort'] # Add others if needed
    ...
    if not df_model1.empty:
        ...
        if df_model1['dep_var_birth_wl'].nunique() < 2: logging.error("Model 1 dependent variable has only one outcome.")
        else:
            try:
            ...
            except np.linalg.LinAlgError as e: logging.error(f"LinAlgError (Singular matrix) fitting Model 1. Try simplifying formula further.")
    ...
    if not df_model2.empty:
        ...
        formula2 = f"dep_var_work_wl ~ {formula_rhs}" # Use same simplified RHS
        ...
        if df_model2['dep_var_work_wl'].nunique() < 2: logging.error("Model 2 dependent variable has only one outcome.")
        else:
            try:
                ...
                if not results2.mle_retvals['converged']: logging.warning("Model 2 failed to converge.")
