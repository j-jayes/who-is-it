...
def main():
    ...
    directors_df.to_csv(os.path.join(OUTPUT_DIR, 'engineer_international_experience.csv'), index=False)
    
    # Save the full data (including position details) to JSON
    with open(os.path.join(OUTPUT_DIR, 'engineer_international_experience_full.json'), 'w', encoding='utf-8') as f:
    ...
    decade_stats.to_csv(os.path.join(OUTPUT_DIR, 'engineer_international_experience_by_decade.csv'), index=False)
    
    # Extract location metadata
    location_metadata = extract_international_locations(directors_data)
    
    # Save location metadata
    with open(os.path.join(OUTPUT_DIR, 'engineer_international_locations_metadata.json'), 'w', encoding='utf-8') as f:
