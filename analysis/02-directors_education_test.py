...
def main():
    ...
    directors_df.to_csv(os.path.join(OUTPUT_DIR, 'directors_education_data.csv'), index=False)
    
    # Perform analysis by decade
    decade_stats = analyze_education_by_decade(directors_data)
    
    # Save the decade analysis
    decade_stats.to_csv(os.path.join(OUTPUT_DIR, 'directors_education_by_decade.csv'), index=False)
    ...
    with open(os.path.join(OUTPUT_DIR, 'education_metadata.json'), 'w', encoding='utf-8') as f:
